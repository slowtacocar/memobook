...
firebase.auth().onAuthStateChanged((user) => {
  if (user) {
        ...
        $("#notes").append(`
          <div class="card">
            ...
            <div class="card-footer">
              <button type="button" class="btn btn-primary" id="openNote${
                doc.id
              }">Open Note</button>
            </div>
          </div>
    ...
    notesRef.onSnapshot((snapshot) => {
      const activeTags = $(".tags > .tag.active")
        .map((i, e) => $(e).html())
        .get();
      $(".tags").html("");
      tags = [];
      snapshot.forEach((doc) => {
        doc.data().tags.forEach((tag, i, tagData) => {
          if (!tags.includes(tag)) {
            ...
            $(".tags").append(
              `<a class="nav-link tag${active}" id="tag-${tags.indexOf(
                tag
              )}" href="#">${tag}</a>`
            );
...
$("#saveNote").click(() => {
  const tagsArray =
    $(".tags-text").val() === "" ? [] : $(".tags-text").val().split(",");
